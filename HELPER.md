...
docker inspect --format='{{.State.Health.Status}}' vibe-server
...
```bash
# Run single command
docker exec vibe-server ls -la /repos
docker exec code-server pwd

# Run as specific user
docker exec -u node vibe-server whoami
docker exec -u root vibe-server cat /etc/os-release

# Execute with environment variables
docker exec -e DEBUG=1 vibe-server npm test
```
...
docker exec vibe-server su - node -c "claude"
```
...
docker stats vibe-server code-server

# View container details
docker inspect vibe-server
...
# Check Docker logs
docker logs vibe-server --tail 50
...
docker inspect --format='{{json .State.Health}}' vibe-server | jq
