...
**📘 For External/Remote Open WebUI setup, see [EXTERNAL_OPENWEBUI.md](EXTERNAL_OPENWEBUI.md)**
...
## External Open WebUI Connection

**📘 Comprehensive Guide:** See [EXTERNAL_OPENWEBUI.md](EXTERNAL_OPENWEBUI.md) for complete setup instructions including:
...
You can also connect an **external Open WebUI** instance (running on a different machine) to the Vibe Stack MCP server.

### HTTP API Connection

For remote or external Open WebUI instances, use the HTTP API:

**Base URL:** `http://YOUR_VIBE_STACK_HOST:4001`

**Available Endpoints:**
- `GET /v1/functions` - List all available MCP tools
- `POST /v1/plan` - Generate task plan
- `POST /v1/tools/vibe_get_board` - Get board state
- `POST /v1/tools/vibe_create_task` - Create task
- `GET /health` - Health check

### Example: Direct HTTP Usage

```bash
# Generate plan via HTTP
...
# Get board state
curl -X POST http://localhost:4001/v1/tools/vibe_get_board \
  -H "Content-Type: application/json" \
  -d '{}'
```

### Docker Exec Connection (Same Machine)

If your external Open WebUI runs on the same host in Docker:
...
|------|-------------|---------|------------|
| `vibe_generate_plan` | Generate task plan from goal | "Create plan for REST API" | goal (required), context, targetLane |
| `vibe_analyze_goal` | Analyze before planning | "Analyze: Add notifications" | goal (required) |
| `vibe_get_context` | Get board status | "What's our context?" | None |

#### Task Management

| Tool | Description | Example | Parameters |
|------|-------------|---------|------------|
| `vibe_create_task` | Create single task | "Add task: Fix login bug" | title (required), description, lane, priority, estimatedHours, tags |
| `vibe_move_task` | Move between lanes | "Move task #123 to done" | taskId, targetLane |
| `vibe_update_task` | Update properties | "Mark #456 as high priority" | taskId, title, description, priority, status, estimatedHours |
| `vibe_search_tasks` | Search tasks | "Find API tasks" | query, lane |
| `vibe_batch_create` | Create multiple tasks | "Add these tasks: ..." | tasks (array) |

#### Information

| Tool | Description | Example | Parameters |
|------|-------------|---------|------------|
| `vibe_get_board` | Get board state | "Show me the board" | None |
| `vibe_get_stats` | Get statistics | "Show metrics" | None |
