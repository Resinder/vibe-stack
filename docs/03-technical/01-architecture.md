...
## MCP Server Architecture (v1.1.8)

### Modular Layer Design (Zero Spaghetti)

The v1.1.8 MCP Server features a clean 5-layer architecture that eliminates code duplication and provides clear separation of concerns with PostgreSQL storage, WebSocket support, and credential management.
...
│
├── mcp-server/                 # MCP Server (v1.1.8 - Modular Architecture)
...
1. **Never commit secrets**
   - `.env` in `.gitignore`
   - Use `.env1.1.8example` as template
...
### Key Features (Current Version v1.1.8)
