...
export class TaskNotFoundError extends AppError {
   ...
   * @param {string} hint - Optional hint for finding the task
   */
  constructor(taskId, hint = null) {
    const message = `Task not found: "${taskId}"${
      hint ? `\nHint: ${hint}` : '\nHint: Use search_tasks to find the correct task ID'
    }`;
    super(
      message,
      'TASK_NOT_FOUND',
...
export class InvalidLaneError extends AppError {
  ...
  constructor(lane, validLanes) {
    const message = `Invalid lane: "${lane}"\n` +
      `Valid lanes are: ${validLanes.join(', ')}\n` +
      `Example: move_task({ taskId: "abc-123", targetLane: "todo" })`;
    super(
      message,
      'INVALID_LANE',
      400,
...
export class BoardError extends AppError {
  ...
  constructor(message, operation = 'unknown', context = {}) {
    const enhancedMessage = `Board operation failed: ${operation}\n${message}${
...
export class PlanningError extends AppError {
  ...
  constructor(message, context = {}) {
    const enhancedMessage = `Planning error: ${message}${
...
export class ErrorHandler {
  ...
  static log(error) {
    ...
    Logger.error('[ErrorHandler]', sanitized);
