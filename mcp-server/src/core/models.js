...
export class Task {
  ...
  constructor(data = {}) {
    ...
    const lane = this._validateAndNormalizeLane(data.lane);
    const priority = this._validateAndNormalizePriority(data.priority);
    this._validateEstimatedHours(data.estimatedHours);
    const tags = this._validateAndNormalizeTags(data.tags);
  ...
  /**
   * Validate and normalize lane value
   * @private
   * @param {*} lane - Lane to validate
   * @returns {string} Normalized lane value
   * @throws {Error} If lane is invalid
   */
  _validateAndNormalizeLane(lane) {
    const normalized = lane || 'backlog';
    if (!Task.VALID_LANES.includes(normalized)) {
      throw new Error(`Invalid lane: ${normalized}. Must be one of: ${Task.VALID_LANES.join(', ')}`);
    }
    return normalized;
  }
   ...
   */
  _validateAndNormalizePriority(priority) {
    const normalized = priority || 'medium';
    if (!Task.VALID_PRIORITIES.includes(normalized)) {
      throw new Error(`Invalid priority: ${normalized}. Must be one of: ${Task.VALID_PRIORITIES.join(', ')}`);
    }
    return normalized;
  }
   ...
   */
  _validateAndNormalizeTags(tags) {
    const normalized = Array.isArray(tags) ? tags : [];
    if (!normalized.every(t => typeof t === 'string')) {
